...
@Slf4j
@SpringBootTest
class AutoLearningServiceImplTest {
    @Autowired
    private AutoLearningServiceImpl autoLearningService;
    @Autowired
    ...
    void predict() {
        ...
        LearningParam param = Learnings.SVMGaussianKernel()
                .setKNodesT(kNodesT)
                .setKNodesP(kNodesP)
                .setFutureNum(6)
                .setSize(20);

        param = Learnings.toLearningParamByKLineH(param);

        LearningRes predict = autoLearningService.fitAndPredict(param);
